...
# sensible defaults if environment is missing or empty
SENDER_EMAIL = _get_env_fallback("APP_EMAIL", "ganeshsai@nuevostech.com")
APP_PASSWORD = _get_env_fallback("APP_PASSWORD", "")  # keep empty if none
...
# Safer build_message that doesn't raise for missing sender
def build_message(sender: str, to_list: list, subject: str, body: str, attachment_path: str = None) -> EmailMessage:
    ...
    # If sender is missing fall back to a sensible default instead of raising
    if not clean_sender:
    ...
    # If no recipients provided, use default recipients
...
def send_email(msg: EmailMessage):
    # If app password or sender missing, skip sending but return False
...
def send_renewal_reminder(agreement: dict, days_left: int):
    ...
    # Attachment path only if file exists
